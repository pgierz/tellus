...
def demo_basic_archive_operations():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            locations = [loc for loc in Location.list_locations() 
                        if any(kind.name in ['DISK', 'COMPUTE'] for kind in loc.kinds)]
            if locations:
                location = locations[0]
                print(f"   Using location: {location.name} ({', '.join(k.name for k in location.kinds)})")
            else:
...
def demo_multi_archive_registry():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for match in matches:
            print(f"   - Archive: {match['archive_name']} (size: {match['size']} bytes)")
...
def demo_path_mapping():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for test_path in test_paths[:2]:  # Just test first 2 files
            try:
                extracted_path = archive.extract_file(test_path, extract_dir, apply_path_mapping=True)
